    ...
    <?php while ($row = $stmt->fetch(PDO::FETCH_ASSOC)): ?>
        ...
        <div class="w-full card p-5">
            <div class="w-full lg:border-r border border-r-gray-300 rounded-md flex flex-col justify-end item-center p-4">
                <div
                    class="w-full h-[10vh]  grid grid-cols-7 place-items-start text-end flex justify-center item-start space-x-5 mb-10 wishcardcontainer">
                    <div
                    ...
                    <div class="col-span-6 text-start flex flex-col space-y-4 self-start bg-blue-00">
                        <span class="self-start leading-relaxed wish-string"  data-original-text="<?php echo $row['wish']; ?>">
                            <?php echo $row['wish']; ?>
                        </span>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    let wishcardcontainer =document.querySelectorAll('.wishcardcontainer');

    for (let x = 0; x < getstrs.length; x++) {
        console.log(getstrs[x].innerHTML);
        var str = getstrs[x].innerHTML;

        if (str.length > 200) {
            // console.log(str.substring(0, 200).concat('...'))
            getstrs[x].innerHTML = str.substring(0, 200).concat(' ......');

           
           getstrs[x].addEventListener('click', function () {
            console.log('hi');
            
            // Retrieve the original text from the data attribute
            let originalText = getstrs[x].getAttribute('data-original-text');
            getstrs[x].innerHTML = originalText;
            wishcardcontainer[x].classList.replace('h-[10vh]','h-full')
        });
