...
try {
    $stmt = $conn->prepare("SELECT photo, name, wish FROM birthdaywish");
    $stmt->execute();
...
try {
    ...
    $stmt = $conn->prepare("SELECT photo, name, wish FROM birthdaywish LIMIT ?, ?");
    ...
    <?php while ($row = $stmt->fetch(PDO::FETCH_ASSOC)): ?>
        ...
        <div class="w-full card p-5">
            <div class="w-full lg:border-r border border-r-gray-300 rounded-md flex flex-col justify-end item-center p-4">
                <div class="w-full h-full grid grid-cols-7 place-items-center text-end flex justify-center item-start space-x-5 mb-10">
                    <div
